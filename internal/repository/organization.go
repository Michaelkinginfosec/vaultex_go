...
func (r *repo) Create(ctx context.Context, organization *model.Organization) error {
	query := `INSERT INTO organizations (organization_name, registered_name, phone_number, website_url,  api_key, api_secret, email, password) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`
...
func (r *repo) FindByEmail(ctx context.Context, email string) (*model.Organization, error) {
	...
	query := `SELECT id, organization_name, registered_name, phone_number, email, api_key FROM organizations WHERE email = $1`
...
func (r *repo) Login(ctx context.Context, email string) (*model.Organization, error) {
	var organization model.Organization
	query := `SELECT id, organization_name, registered_name, phone_number, email, api_key, password FROM organizations WHERE email = $1`
